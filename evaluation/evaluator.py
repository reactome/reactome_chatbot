...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Load a test set and evaluate answers generated by a language model.')
    parser.add_argument('--testset_path', type=str, required=True, help='Path to the testset JSON file')
    parser.add_argument('--model', type=str, default="gpt-3.5-turbo-0125", help='Language model to use for evaluation')
    parser.add_argument('--metrics', type=str, nargs='+', help='List of evaluation metrics (e.g., precision, recall)')
    parser.add_argument('--replicates', type=int, default=1, help='Number of replicates for answer generation')
...
def main():
    ...
    for dataset in all_replicates_results:
        metrics_to_use = [eval(metric) for metric in args.metrics]  # Assuming metrics are safely evaluable
        result = evaluate(
