...
# from src.embeddings.alliance_generator import generate_alliance_embeddings
from src.embeddings.reactome_generator import generate_reactome_embeddings
...
def make(
    embedding: EmbeddingSelection, openai_key: str = None, hf_key: str = None, **kwargs
):
    embedding_path: Path = embedding.path(check_exists=False)
    if openai_key:
...
def ls():
    for embedding_str in EM_ARCHIVE.glob("*/*/*/*"):
        print(embedding_str.relative_to(EM_ARCHIVE))
...
if __name__ == "__main__":
    ...
    make_parser.add_argument("--openai-key", help="API key for OpenAI")
