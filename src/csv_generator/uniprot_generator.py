...
class UniProtDataCleaner:
    ...
    def format_names(self):
        ...
        self.df['Gene Names'] = self.df['Gene Names'].str.replace(' ', '; ', regex=False).str.strip('; ')
    ...
    def clean_columns(self):
        ...
        def reformat_domain(entry):
            ...
            matches = re.findall(pattern, entry)
            return '; '.join([f"Has a {note} domain at position {pos.replace('..', '-')}" for pos, note in matches])
