...
class UniProtRetriever:
    ...
    def __init__(
        ...
        self.embeddings_directory = embeddings_directory or EmbeddingEnvironment.get_dir("uniprot")
        
        if not self.embeddings_directory.exists():
            raise ValueError(f"Embeddings directory does not exist: {self.embeddings_directory}")
    ...
    def _initialize_retrievers(self) -> None:
        ...
        logger.info(f"UniProt retriever initialized successfully using subdirectory: {self._subdirectory}")
    ...
    async def ainvoke(
        ...
        try:
            logger.info(f"UniProt retrieve called with query='{query}', expanded_queries={expanded_queries}, use_rrf={cfg.vector_config.use_rrf}")
            
            if cfg.vector_config.use_rrf and expanded_queries and len(expanded_queries) > 1:
    ...
    async def _search_with_rrf(
        ...
        for expanded_query in expanded_queries:
            tasks.append(self._search_vectorstore(
                expanded_query, 
        ...
        for i, ranked_list in enumerate(ranked_lists):
            ...
            if ranked_list:
                ...
                doc_id = first_doc.metadata.get("url", first_doc.metadata.get("id", hash(first_doc.page_content)))
                logger.info(f"  First result ID: {doc_id}")
                logger.info(f"  First result content: {first_doc.page_content[:100]}...")
        
        # Apply RRF to combine all ranked lists
        logger.info(f"Applying RRF with final_k={cfg.vector_config.rrf_final_k}, lambda={cfg.vector_config.rrf_lambda}")
        top_docs, _, _ = reciprocal_rank_fusion(
            ...
            id_getter=lambda doc: doc.metadata.get("url", doc.metadata.get("id", hash(doc.page_content))),
        )
    ...
    async def _search_simple(self, query: str, cfg: UniProtRetrievalConfig) -> List[str]:
    ...
    def is_initialized(self) -> bool:
        ...
        return self._vectorstore is not None and (self._bm25_retriever is not None or self._vectorstore is not None)
