...
def upload_to_chromadb(
    ...
    metadata_columns: dict[str, list] = {
        ...
        "complexes": ["st_id", "display_name", "component_id", "component_name", "species"],
        "ewas": [
            "st_id",
            "display_name",
            "canonical_gene_name",
...
def generate_reactome_embeddings(
    ...
    all_exist = not force and all(Path(p).exists() for p in [reactions_csv, summations_csv, complexes_csv, ewas_csv])
