...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the directory containing example files",
    )
    parser.add_argument(
        "--model",
        type=str,
        default="gpt-4-turbo",
        help="Language model to use (default: gpt-4-turbo)",
    )
    parser.add_argument(
        "--temperature",
        type=float,
        default=0.3,
        help="Temperature setting for the language model (default: 0.3)",
    )
    parser.add_argument(
        "--test_size",
        type=int,
        default=4,
        help="Number of tests to generate (default: 4)",
    )
    parser.add_argument(
        "--distributions",
        type=str,
        nargs="+",
        help="Distributions of test types (e.g., simple=0.25 reasoning=0.25 multi_context=0.25 conditional=0.25)",
    )
...
def main():
    try:
        ...
        testset = generator.generate_with_langchain_docs(
            documents, test_size=args.test_size, distributions=distributions
        )

        # Save the output
        base_filename = os.path.basename(args.path).split(".")[
            0
