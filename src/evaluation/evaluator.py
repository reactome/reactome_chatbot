...
def parse_arguments():
    ...
    parser.add_argument(
        "--testset_dir", type=str, required=True, help="Path to the directory containing testset Excel (.xlsx) files"
    )
    parser.add_argument(
        "--model",
        type=str,
...
def initialize_rag_chain_with_memory(embeddings_directory, model_name):
    ...
    loader = CSVLoader("/Users/hmohammadi/Desktop/react_to_me_github/reactome_chatbot/embeddings/openai/text-embedding-3-large/reactome/summation_csv/summations.csv")
    data = loader.load()
...
def process_testset(testset_path, qa_system, embeddings_directory, response_dir, eval_dir, model_name):
    ...
    response_filename = os.path.join(
        response_dir, f"{os.path.splitext(os.path.basename(testset_path))[0]}_{args.model}_responses.xlsx"
    )
    ...
    evaluation_filename = os.path.join(
        eval_dir, f"{os.path.splitext(os.path.basename(testset_path))[0]}_{args.model}_evaluation.xlsx"
    )
...
def main():
    ...
    for filename in os.listdir(args.testset_dir):
        ...
        if filename.endswith(".xlsx"):
            ...
            process_testset(testset_path, qa_system, embeddings_directory, response_dir, eval_dir, model_name)
